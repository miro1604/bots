...
Output:
  finance/debate/debate_log.jsonl - kaikki kierrokset
...
PERSONAS_FILE = _FIN_ROOT / "agents" / "personas.jsonl"
DEBATE_LOG = _FIN_ROOT / "debate" / "debate_log.jsonl"
PROPOSED = _FIN_ROOT / "signals" / "proposed_signals.jsonl"
CONSENSUS = _FIN_ROOT / "signals" / "consensus_signals.jsonl"
...
def load_persona_prompt(persona: dict) -> str:
    p = _FIN_ROOT / persona["prompt_file"]
...
"""
        ...
        out, err, rc = call_claude(
            ...
            model="sonnet", fallback_model="haiku", bot=f"finance:{persona['slug']}"
        )
        dur = time.time() - t0
        if rc != 0:
            print(f" FAIL rc={rc}")
